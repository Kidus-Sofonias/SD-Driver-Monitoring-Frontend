...
export function RoutePreview({ points, snappedPoints = [], events = [], height = 220, showLegend = true }: Props) {
  ...
  const cleanedSnappedPoints = useMemo(() => cleanRoutePoints(snappedPoints), [snappedPoints]);
  ...
  const snappedCoordinates = useMemo(
    () => cleanedSnappedPoints.map((point) => [point.lat, point.lon] as [number, number]),
    [cleanedSnappedPoints]
  );
...
function LeafletRouteMap({
  ...
  const hasSnapped = snappedCoordinates.length >= 2;
  const displayCoordinates = hasSnapped ? snappedCoordinates : routeCoordinates;
  const snapMode: "snapped" | "cleaned" = hasSnapped ? "snapped" : "cleaned";
    ...
    <View style={[styles.routeCanvasShell, { backgroundColor: colors.panelRaised, borderColor: colors.line }]}>
      <View style={[styles.statusBanner, { backgroundColor: "#E7F6EC", borderColor: "#B7DEC4" }]}>
        ...
        <Text style={[styles.statusText, { color: "#1B6B37" }]}>
          {snapMode === "snapped"
            ? "Road-snapped route is active for a more realistic path."
            : "Using cleaned GPS route (road snapping unavailable)."}
        </Text>
      </View>
      <View style={styles.leafletHost}>
        <bundle.MapContainer
          bounds={displayCoordinates}
        ...
        >
          ...
          />
          <bundle.Polyline positions={displayCoordinates} pathOptions={{ color: "#1677FF", weight: 4 }} />
          {startPoint ? (
      ...
      {showLegend ? (
        <View style={styles.routeLegend}>
          <Text style={[styles.routeLegendText, { color: colors.text }]}>
            Blue line = {snapMode === "snapped" ? "road-snapped route" : "cleaned route"}
          </Text>
          <Text style={[styles.routeLegendText, { color: colors.text }]}>Green = start</Text>
          <Text style={[styles.routeLegendText, { color: colors.text }]}>Red = finish</Text>
...
function eventTooltip(event: DrivingEvent) {
  ...
  return `${humanizeEventType(event.event_type)} - value ${Math.round(event.value)} at ${new Date(occurredAt).toLocaleString()}`;
}
